...
def detect_face_and_extract_skin(img_bgr):
    ...
    if face_crop_gate3.size > 0:
        ...
        if hf_mean < 4.5 and edge_density < 0.08:
            return None, "not_human"
    # ── End Gate 3b ───────────────────────────────────────────────────────
    ...
    if face_crop.size > 0:
        ...
        # Human faces always have lips, eyes, shadows → max channel std > 25
        # Teddy bears / plush toys: all channels uniform → max std < 20
        if max(ch_stds) < 20.0:
            return None, "not_human"
        # Monochromatic check: human faces have R channel significantly higher than B
        # (lips, skin warmth). Plush toys and dolls move all channels together.
        if min(ch_stds) > 0 and max(ch_stds) / min(ch_stds) < 1.5:
            if max(ch_stds) < 40.0:
                return None, "not_human"
